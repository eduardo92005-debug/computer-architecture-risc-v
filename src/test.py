...
def read_hex_file(file_path):
    """
    Reads a hexadecimal file and returns the hexadecimal content as a string.
    """
...
def process_file(file_path):
    """
    Reads a hexadecimal file, converts each hexadecimal value to binary, 
    and then decodes the instructions.
    """
    hex_content = read_hex_file(file_path)

    # Split the content into 32-bit chunks (assuming each instruction is 4 bytes)
    instructions = [hex_content[i:i+8] for i in range(0, len(hex_content), 8)]

    for instruction in instructions:
        binary_instr = hex_to_bin(instruction)
        print(analyse(binary_instr))
